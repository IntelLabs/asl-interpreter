...
TEST_F(Memory, Init)
{
    ASL_ram_init(address_size, init_size, memory, 0);
    ASL_ram_init(address_size, init_size, memory, 0xff);
}

TEST_F(Memory, Read)
{
    uint64_t val = 0xab;
    ASL_ram_init(address_size, init_size, memory, val);
    EXPECT_EQ(val, ASL_ram_read(address_size, 1, memory, 0));
}
