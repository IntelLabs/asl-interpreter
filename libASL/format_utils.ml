...
let sepby (fmt: PP.formatter) (sep: unit -> unit) (pp: 'a -> unit) (xs: 'a list): unit =
    PP.pp_print_list ~pp_sep: (fun _ -> sep) (fun _ -> pp) fmt xs

let with_color (fmt: PP.formatter) (color: ColorT.color4) (pp: unit -> unit): unit =
    Color.pp_open_style fmt (Fg (C4 color));
    pp ();
    Color.pp_close_style fmt ()

let cutsep (fmt: PP.formatter) (pp: 'a -> unit) (xs: 'a list): unit = sepby fmt (fun _ -> cut fmt) pp xs
let map (fmt: PP.formatter) (pp: 'a -> unit) (xs: 'a list): unit = sepby fmt (fun _ -> ()) pp xs
let hlist (fmt: PP.formatter) (sep: unit -> unit) (pp: 'a -> unit) (xs: 'a list): unit = sepby fmt (fun _ -> nbsp fmt) pp xs

let surround (fmt: PP.formatter) (l: PP.formatter -> unit) (r: PP.formatter -> unit) (m: unit -> unit) = l fmt; m(); r fmt
