...
let loc (fmt: PP.formatter) (x: AST.l): unit = PP.pp_print_string fmt (AST.pp_loc x)
let delimiter (fmt: PP.formatter) (s: string): unit = with_color fmt ColorT.magenta (fun _ -> PP.pp_print_string fmt s)
let keyword (fmt: PP.formatter) (s: string): unit = with_color fmt ColorT.red (fun _ -> PP.pp_print_string fmt s)
let constant (fmt: PP.formatter) (s: string): unit = with_color fmt ColorT.blue (fun _ -> PP.pp_print_string fmt s)

let ident     (fmt: PP.formatter) (color: ColorT.color4) (x: AST.ident): unit = with_color fmt color (fun _ -> PP.pp_print_string fmt (AST.pprint_ident x))
let tycon     (fmt: PP.formatter) (x: AST.ident): unit = ident fmt ColorT.green x
let funname   (fmt: PP.formatter) (x: AST.ident): unit = ident fmt ColorT.hi_cyan x
let varname   (fmt: PP.formatter) (x: AST.ident): unit = ident fmt ColorT.cyan x
let fieldname (fmt: PP.formatter) (x: AST.ident): unit = ident fmt ColorT.yellow x
...
let kw_underscore_instruction          (fmt: PP.formatter): unit = keyword fmt "__instruction"
let kw_underscore_instruction_set      (fmt: PP.formatter): unit = keyword fmt "__instruction_set"
...
let insert_comment (fmt: PP.formatter) (p: comment -> bool): unit =
    ...
    | [(_, _, c)] -> nbsp fmt; PP.pp_print_string fmt c
...
let insert_comments (fmt: PP.formatter) (p: comment -> bool): unit =
    ...
    | cs -> vbox fmt (fun _ -> map fmt (fun (s, f, c) -> PP.pp_print_string fmt c; cut fmt) cs)
    )
...
let funnames  (fmt: PP.formatter) (xs: AST.ident list): unit = commasep fmt (funname fmt) xs
...
let binop (fmt: PP.formatter) (x: AST.binop): unit =
  ...
  | Binop_DUMMY       -> PP.pp_print_string fmt "Binop_DUMMY" (* todo: throw an error? *)
  )
...
let rec ty (fmt: PP.formatter) (x: AST.ty): unit =
  ...
  | Type_Array(ixty, ety) ->
    ...
    nbsp fmt; kw_of fmt; nbsp fmt;
    ty fmt ety
  | Type_Tuple(tys) -> parens fmt (fun _ -> types fmt tys)
  )

and types (fmt: PP.formatter) (tys: AST.ty list): unit = commasep fmt (ty fmt) tys
...
and slices (fmt: PP.formatter) (ss: AST.slice list): unit = commasep fmt (slice fmt) ss
...
and expr (fmt: PP.formatter) (x: AST.expr): unit = 
  (match x with
  | Expr_If(c, t, els, e) ->
    kw_if fmt; nbsp fmt; expr fmt c; nbsp fmt; kw_then fmt;
    map fmt
      (fun (AST.E_Elsif_Cond (c, e)) -> nbsp fmt; kw_elsif fmt; nbsp fmt; expr fmt c; nbsp fmt; kw_then fmt; nbsp fmt; expr fmt e)
      els;
    nbsp fmt; kw_else fmt; nbsp fmt; expr fmt e
  | Expr_Binop(a, op, b) -> expr fmt a; nbsp fmt; binop fmt op; nbsp fmt; expr fmt b
  ...
  | Expr_TApply(f, tes, es) ->
    funname fmt f;
    if show_tyargs then begin lbrace_lbrace fmt; exprs fmt tes; rbrace_rbrace fmt end;
...
and patterns (fmt: PP.formatter) (ps: AST.pattern list): unit = commasep fmt (pattern fmt) ps

let rec lexpr (fmt: PP.formatter) (x: AST.lexpr): unit =
  ( match x with
  | LExpr_Wildcard -> minus fmt
  | LExpr_Var(v) -> varname fmt v
  | LExpr_Field(e, f) -> lexpr fmt e; dot fmt; fieldname fmt f
  | LExpr_Fields(e, fs) ->
    lexpr fmt e; dot fmt;
    brackets fmt (fun _ -> commasep fmt (fieldname fmt) fs)
  | LExpr_Slices(e, ss) -> lexpr fmt e; brackets fmt (fun _ -> slices fmt ss)
  | LExpr_BitTuple(es)  -> brackets fmt (fun _ -> lexprs fmt es)
  | LExpr_Tuple(es) -> parens fmt (fun _ -> lexprs fmt es)
  | LExpr_Array(a, e) -> nbsp fmt; lexpr fmt a; brackets fmt (fun _ -> expr fmt e)
  | LExpr_Write(f, tes, es) ->
    kw_underscore_write fmt; nbsp fmt;
    funname fmt f;
    if show_tyargs then begin lbrace_lbrace fmt; exprs fmt tes; rbrace_rbrace fmt end;
    parens fmt (fun _ -> exprs fmt es)
  | LExpr_ReadWrite(f, g, tes, es) ->
    kw_underscore_readwrite fmt; nbsp fmt;
    funname fmt f;
    nbsp fmt;
    funname fmt g;
    if show_tyargs then begin lbrace_lbrace fmt; exprs fmt tes; rbrace_rbrace fmt end;
...
let rec stmt (fmt: PP.formatter) (x: AST.stmt): unit =
  ...
  | Stmt_Assign (l, r, loc) ->
    comments_before fmt loc;
    lexpr fmt l; nbsp fmt; eq fmt; nbsp fmt; expr fmt r; semicolon fmt;
    comment_start fmt loc
  ...
  | Stmt_DecodeExecute (i, e, loc) ->
    comments_before fmt loc;
    kw_underscore_decode fmt; nbsp fmt; varname fmt i; nbsp fmt; expr fmt e; semicolon fmt;
    comment_start fmt loc
  | Stmt_Block (ss, loc) ->
    comments_before fmt loc;
    lbrace fmt;
    ...
    vbox fmt (fun _ ->
      ...
      map fmt
        (fun (AST.S_Elsif_Cond (c, s, loc)) ->
          cut fmt;
          kw_elsif fmt; nbsp fmt; expr fmt c; nbsp fmt; kw_then fmt;
          comment_start fmt loc;
  ...
  | Stmt_Case (e, alts, ob, loc) ->
    comments_before fmt loc;
    vbox fmt (fun _ ->
      kw_case fmt; nbsp fmt; expr fmt e; nbsp fmt; kw_of fmt;
      comment_start fmt loc;
      indented fmt (fun _ ->
        cutsep fmt (fun (AST.Alt_Alt(ps, oc, ss, loc)) ->
          kw_when fmt; nbsp fmt; patterns fmt ps;
  ...
  | Stmt_For (v, f, dir, t, b, loc) ->
    comments_before fmt loc;
    kw_for fmt; nbsp fmt; varname fmt v;
    nbsp fmt; eq fmt; nbsp fmt;
    expr fmt f;
  ...
  | Stmt_Repeat (b, c, pos, loc) ->
    ...
    indented_block fmt b;
    cut fmt;
    kw_until fmt; nbsp fmt; expr fmt c; semicolon fmt;
  ...
  | Stmt_Try (b, v, pos, cs, ob, loc) ->
    ...
    comment_start fmt loc;
    indented_block fmt b;
    cut fmt; kw_catch fmt; nbsp fmt; varname fmt v;
    comment_here fmt pos;
    indented fmt (fun _ ->
      cutsep fmt (fun (AST.Catcher_Guarded (e, b, loc)) ->
        kw_when fmt; nbsp fmt; expr fmt e;
        comment_start fmt loc;
        indented_block fmt b
      ) cs;
...
let instr_field (fmt: PP.formatter) (x: AST.instr_field): unit =
  ...
  | IField_Field(f, lo, wd) ->
    kw_underscore_field fmt; nbsp fmt; fieldname fmt f; nbsp fmt;
    int fmt lo; nbsp fmt; plus_colon fmt; nbsp fmt; int fmt wd
  )

let opcode_value (fmt: PP.formatter) (x: AST.opcode_value): unit =
  (match x with
  | Opcode_Bits b -> bitsLit fmt b
  | Opcode_Mask m -> maskLit fmt m
  )

let encoding (fmt: PP.formatter) (x: AST.encoding): unit =
  (match x with
  | Encoding_Block (nm, iset, fs, op, e, ups, b, loc) ->
    kw_underscore_encoding fmt; nbsp fmt; varname fmt nm;
    indented fmt (fun _ ->
      kw_underscore_instruction_set fmt; nbsp fmt; varname fmt iset;
      cut fmt;
      map fmt (fun f -> instr_field fmt f; cut fmt) fs;
      kw_underscore_opcode fmt; nbsp fmt; opcode_value fmt op;
      cut fmt;
      kw_underscore_guard fmt; nbsp fmt; expr fmt e;
      cut fmt;
      map fmt (fun (i, b) ->
        kw_underscore_unpredictable_unless fmt; nbsp fmt;
        int fmt i; nbsp fmt; eq_eq fmt; nbsp fmt; bitsLit fmt b;
        cut fmt
      ) ups;
      kw_underscore_decode fmt; indented_block fmt b
    )
  )

let decode_slice (fmt: PP.formatter) (x: AST.decode_slice): unit =
  (match x with
  | DecoderSlice_Slice (lo, wd) -> int fmt lo; nbsp fmt; plus_colon fmt; nbsp fmt; int fmt wd
...
let rec decode_pattern (fmt: PP.formatter) (x: AST.decode_pattern): unit =
  ...
  | DecoderPattern_Not p -> bang fmt; decode_pattern fmt p
  )
...
let rec decode_body (fmt: PP.formatter) (x: AST.decode_body): unit =
  ...
  | DecoderBody_Encoding (e, loc) -> kw_underscore_encoding fmt; nbsp fmt; varname fmt e
...
and decode_case (fmt: PP.formatter) (x: AST.decode_case): unit =
  ...
  | DecoderCase_Case (ss, alts, loc) ->
    kw_case fmt; nbsp fmt;
    parens fmt (fun _ -> commasep fmt (decode_slice fmt) ss);
    nbsp fmt; kw_of fmt;
    indented fmt (fun _ ->
      cutsep fmt (fun (AST.DecoderAlt_Alt(ps, b)) ->
        kw_when fmt;
...
let declaration (fmt: PP.formatter) (x: AST.declaration): unit =
  ...
  | Decl_BuiltinType (tc, loc) ->
    comments_before fmt loc;
    kw_underscore_builtin fmt; nbsp fmt;
    kw_type fmt; nbsp fmt; tycon fmt tc; semicolon fmt
  | Decl_Forward (tc, loc) ->
    comments_before fmt loc;
    kw_type fmt; nbsp fmt; tycon fmt tc; semicolon fmt
  | Decl_Record (tc, fs, loc) ->
    ...
    braces fmt (fun _ ->
      indented fmt (fun _ ->
        cutsep fmt (fun (f, t) ->
            fieldname fmt f; nbsp fmt; ty fmt t; semicolon fmt
        ) fs
      );
      cut fmt
    );
  ...
  | Decl_BuiltinFunction (f, ps, args, t, loc) ->
    ...
    function_header fmt (Some t) f ps (fun _ -> formals fmt args); semicolon fmt
  ...
  | Decl_ProcType (f, ps, args, loc) ->
    comments_before fmt loc;
    kw_func fmt; nbsp fmt;
    function_header fmt None f ps (fun _ -> formals fmt args); semicolon fmt
  ...
  | Decl_Operator2 (op, fs, loc) ->
    comments_before fmt loc;
    kw_underscore_operator2 fmt; nbsp fmt; binop fmt op;
    nbsp fmt; eq fmt; nbsp fmt;
    funnames fmt fs;
    semicolon fmt
  ...
  | Decl_MapClause(f, fs, oc, b, loc) ->
    ...
    funname fmt f;
    parens fmt (fun _ ->
      commasep fmt (fun (AST.MapField_Field (f, p)) ->
        varname fmt f; nbsp fmt; eq fmt; nbsp fmt; pattern fmt p
      ) fs
    );
...
let declarations (fmt: PP.formatter) (xs: AST.declaration list): unit =
  vbox fmt (fun _ -> map fmt (fun d -> declaration fmt d; cut fmt) xs)
