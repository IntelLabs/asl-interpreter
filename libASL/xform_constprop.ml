...
module Env : sig
    ...
    let globals (env: t): Eval.GlobalEnv.t =
        env.globalConsts

    let locals (env: t): value ScopeStack.t =
        env.locals
    ...
    let seq (m: t -> 'a) (k: t -> 'b) (env: t): ('a * 'b) =
        ...
        let b = k env in
        env.return_value <- Values.lub env.return_value r; (* REVISIT *)
        (a, b)
    ...
    let getVar (env: t) (x: ident): value =
        ...
        from_option (map_option Values.singleton (Eval.GlobalEnv.getGlobalConstOpt env.globalConsts x)) (fun _ ->
...
let rec value_to_expr (x: Concrete.value): expr option =
    ...
    *)
    ...
    | VTuple vs -> map_option (fun es -> Expr_Tuple es) (flatten_map_option value_to_expr vs)
    | _ -> None
    )
...
let algebraic_simplifications (x: expr): expr =
    ...
    (* [] == '' *)
    | Expr_Concat(xs) ->
         let xs' = List.filter (function Expr_LitBits "" -> false | _ -> true) xs in
         if xs' = [] then Expr_LitBits "" else Expr_Concat(xs')

    (* '' : x == x == x : '' *)
    | Expr_TApply(FIdent ("append_bits", _), [Expr_LitInt "0"; _], [_; y]) -> y
    | Expr_TApply(FIdent ("append_bits", _), [_; Expr_LitInt "0"], [x; _]) -> x
...
class constEvalClass (env: Env.t) = object (self)
        ...
        | _ ->
            (try
                let eval (x: expr): expr =
                    if isConstant x then begin
                        x
                    end else if isPure(x) && List.for_all isConstant (subexprs_of_expr x) then begin
                        let env0 = Env.to_concrete env in
                        let x' = from_option (value_to_expr (Eval.eval_expr Unknown env0 x)) (fun _ -> x) in
                        (*
...
let xform_slice (env: Env.t) (x: AST.slice): AST.slice =
    (match x with
    | Slice_Single(e) ->
        Slice_Single(xform_expr env e)
    | Slice_HiLo(hi, lo) ->
        Slice_HiLo(xform_expr env hi, xform_expr env lo)
    | Slice_LoWd(lo, wd) ->
        Slice_LoWd(xform_expr env lo, xform_expr env wd)
    )
...
let rec xform_lexpr (env: Env.t) (x: AST.lexpr) (r: Values.t): AST.lexpr =
    ...
    *)
    ...
    | LExpr_Write(setter, tes, es) ->
            ...
            LExpr_Write(setter, tes', es')
    | _ ->
            failwith ("xform_lexpr: "^ pp_lexpr x)
    )
...
and xform_stmt (env: Env.t) (x: AST.stmt): AST.stmt list =
    ...
    *)
    | Stmt_Block(ss, loc) ->
         [Stmt_Block(xform_stmts env ss, loc)]
    | Stmt_If(c, t, els, (e, el), loc) ->
            ...
            | ([], e') -> e'
            | (S_Elsif_Cond(c', t', loc) :: els', e') -> [Stmt_If(c', t', els', (e', el), loc)]
            )
    | Stmt_Case(e, alts, odefault, loc) ->
            ...
            let rec xform env alts =
                (match alts with
                | [] ->
                        let odefault' = map_option (fun (s, loc) -> (xform_stmts env s, loc)) odefault in
                        ([], odefault')
                | (Alt_Alt(ps, oc, s, loc) :: alts') ->
                        let ps' = xform_patterns env ps in
                        let oc' = map_option (xform_expr env) oc in
                        let (s', (alts'', odefault')) = Env.fork_join
                            (fun env -> xform_stmts env s)
    ...
    *)
    | _ ->
        failwith ("xform_stmt: "^ pp_stmt x)
    )

let xform_tvs (env: Env.t) (loc: AST.l) (f: AST.ident): unit =
    let genv = Env.globals env in
    let tvs = ( match Eval.GlobalEnv.getFun loc genv f with
              | Some (tvs, _, _, _) -> tvs
...
let xform_decls (genv: Eval.GlobalEnv.t) (ds: AST.declaration list): AST.declaration list =
    let isConstant (v: ident): bool = Option.is_some (Eval.GlobalEnv.getGlobalConstOpt genv v) in
    let isImpurePrim (v: ident): bool = List.exists (fun p -> Id.matches p v) Concrete.impure_prims in
