...
let rec getFun (f: AST.ident) (ds: AST.declaration list): AST.declaration option =
    (match ds with
    | [] -> None
...
module InstanceKey = struct
    ...
    let compare (x: t) (y: t): int =
        ( AST.Id.compare (fst x) (fst y)
        <?> (List.compare Z.compare, snd x, snd y)
        )
end

module Instances = Map.Make(InstanceKey)

class monoClass (genv: Eval.GlobalEnv.t) (ds: AST.declaration list) = object(self)
    ...
    method monomorphize (genv: Eval.GlobalEnv.t) (f: AST.ident) (d: AST.declaration) (szs: Z.t list): AST.ident option =
        let tvs = (match Eval.GlobalEnv.getFun Unknown genv f with
            | Some (tvs, _, _, _) -> tvs
            | _ -> failwith "monomorphize"
            )
        in
        assert (List.length tvs = List.length szs);
        let suffices = List.map2 (fun nm sz -> AST.pprint_ident nm ^"_"^ Z.to_string sz) tvs szs in
        let f' = AST.addSuffix f (String.concat "_" suffices) in
        let key = (f, szs) in
        if Instances.mem key instances then
            Some f'
        else (
            Printf.printf "Monomorphizing %s to %s\n" (AST.pprint_ident f) (AST.pprint_ident f');

            let env = Xform_constprop.Env.newEnv genv in
            List.iter2 (fun sz tv -> Xform_constprop.Env.addLocalConst env tv (Xform_constprop.Values.singleton (VInt sz))) szs tvs;
            ...
            | Decl_ArrayGetterDefn (f, ps, atys, rty, body, loc) ->
                ...
                let atys' = List.map (fun (v, ty) -> (v, Xform_constprop.xform_ty env ty)) atys in
                let body' = Xform_constprop.xform_stmts env body in
                let d' = AST.Decl_ArrayGetterDefn(f', ps, atys', rty', body', loc) in
                let d' = visit_decl (self :> aslVisitor) d' in
                instances <- Instances.add key d' instances;
                Some f'
        ...
        | Expr_TApply(f, tys, args) ->
            ...
            | Some sizes ->
                Utils.from_option (
                    ...
                    Utils.bind_option (self#monomorphize genv f d sizes) (fun f' ->
                    Some (ChangeDoChildrenPost (AST.Expr_TApply (f', [], args), Fun.id))
                    )))
                    (fun _ -> DoChildren)
            | None ->
                DoChildren
        ...
        | LExpr_Write(f, tes, es) ->
            ...
            | Some sizes ->
                Utils.from_option (
                    Utils.bind_option (getFun f ds) (fun d ->
                    Utils.bind_option (self#monomorphize genv f d sizes) (fun f' ->
                    Some (ChangeDoChildrenPost (AST.LExpr_Write (f', [], es), Fun.id))
                    )))
                    (fun _ -> DoChildren)
            | None ->
                DoChildren
        ...
        | Stmt_TCall (f, tys, args, loc) ->
            ...
            | Some sizes ->
                Utils.from_option (
                    Utils.bind_option (getFun f ds) (fun d ->
                    Utils.bind_option (self#monomorphize genv f d sizes) (fun f' ->
                    Some (ChangeDoChildrenPost (AST.Stmt_TCall (f', [], args, loc), Fun.id))
                    )))
                    (fun _ -> DoChildren)
            | None ->
                DoChildren
