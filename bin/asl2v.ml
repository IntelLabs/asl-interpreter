...
let main () =
    ...
    if true then Utils.to_file "tmp.init0.asl" (fun fmt -> ASL_FMT.declarations fmt ds);
    let ds = Asl_utils.reachable_decls (List.append roots keeps) ds in
    if ds = [] then failwith "Couldn't find any roots";

    (try
        if !output_file == "" then failwith "Output file not specified (use -o foo.v to specify)";

        if true then Utils.to_file "tmp.init.asl" (fun fmt -> ASL_FMT.declarations fmt ds);

        let genv = Eval.build_constant_environment ds in
        let ds = CP.xform_decls genv ds in

        if true then Utils.to_file "tmp.constprop.asl" (fun fmt -> ASL_FMT.declarations fmt ds);

        let ds = Xform_mono.monomorphize ds in
        if true then Utils.to_file "tmp.mono.asl" (fun fmt -> ASL_FMT.declarations fmt ds);
